...
void Ins_eskf_ROS_Wrapper::register_sub_pub(){
    ...
    sub_uwb = nh.subscribe<nlink_parser::LinktrackNodeframe2>(uwb_topic,100,&Ins_eskf_ROS_Wrapper::uwb_cb,this);
    pub_uwb_dis = nh.advertise<std_msgs::Float64>("/uwb_dis",50);
    ...
    if(!initialized){
        ...
        if(dataset == "kitti"){
            if(uwb_buf.size() > 0 && imu_buf.size() > 10 /*&& init_twist_buf.size() > 0*/){
                ...
                initialized = true;
                p_ins_eskf->specify_init_state(init_state);
...
*/
//UWB信号对应的消息msgs
void Ins_eskf_ROS_Wrapper::uwb_cb(const nlink_parser::LinktrackNodeframe2ConstPtr& uwb_in){
...
void Ins_eskf_ROS_Wrapper::initialization_kitti(){
    ...
    */
    ...
    double uwb_data_stamp = uwb_buf.back().local_time;
    //double gps_data_stamp = gps_buf.back().header.stamp.toSec();
    /* 改为直接将最晚的IMU数据作为UWB数据时间戳对齐数据
    ...
    */
    int imu_data_index = imu_buf.size();
    ...
    initial_stamp_uwb = uwb_data_stamp;
    initial_stamp_imu = imu_buf[imu_data_index - 1].header.stamp.toSec();
...
bool Ins_eskf_ROS_Wrapper::synce_measure(){
    ...
    */
    ...
    pub_uwb_dis.publish(float_to_stdmsgs_Float64(measure.uwb_data.range));
    //gps_buf.pop_front();
    uwb_buf.pop_front();
...
Ins_eskf::UWB_data Ins_eskf_ROS_Wrapper::uwb_msg_2_data(nlink_parser::LinktrackNodeframe2 _uwb_msg){

    Ins_eskf::UWB_data uwb_data_;

    uwb_data_.stamp = uwb_stamp_synce(_uwb_msg.local_time);
    if (_uwb_msg.nodes.empty()){
        uwb_data_.range = 0;
    }
    else{
        uwb_data_.range = _uwb_msg.nodes[0].dis;
    }
...
Ins_eskf::IMU_data Ins_eskf_ROS_Wrapper::imu_msg_2_data(sensor_msgs::Imu _imu_msg){
    ...
    imu_data_.stamp = imu_stamp_synce(_imu_msg.header.stamp.toSec());
...
void Ins_eskf_ROS_Wrapper::visualize_res_and_kitti_gps_magnetormeter(){
    ...
    */
    ...
    Ins_eskf::State current_normal_state_ = p_ins_eskf->get_normal_state();
    ...
    nav_msgs::Odometry imu_normal_odom_ = state_to_odom_msg(current_normal_state_,state_stamp);
    
    //将kitti_gps_odom_msg_以及 imu_odom_发布出去
    pub_imu_odometrty_.publish(imu_odom_);
    pub_imu_normal_odometrty.publish(imu_normal_odom_);
    //pub_kitti_gps_odometrty_.publish(kitti_gps_odom_msg_);

}

std_msgs::Float64 Ins_eskf_ROS_Wrapper::float_to_stdmsgs_Float64(float range){
    std_msgs::Float64 uwb_dis;
    uwb_dis.data = range;
