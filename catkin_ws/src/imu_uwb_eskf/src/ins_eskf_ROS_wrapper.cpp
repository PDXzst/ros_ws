#include "ins_eskf_ros_wrapper.h"
...
void Ins_eskf_ROS_Wrapper::register_sub_pub(){
    LOG(INFO) << "topic subscribe and advertise.";
...
void Ins_eskf_ROS_Wrapper::gps_cb(const sensor_msgs::NavSatFixConstPtr& gps_in){
    ...
    if(synce_measure()){
        ...
        // 主要用于将组合导航的结果进行可视化的验证  与kitti数据集中的GPS+九轴IMU姿态的组合在一起的结果进行比较
...
void Ins_eskf_ROS_Wrapper::imu_cb(const sensor_msgs::ImuConstPtr& imu_in){
    ...
    if(!initialized){
        ...
        if(dataset == "kitti"){
            if(uwb_buf.size() > 0 && imu_buf.size() > 10 /*&& init_twist_buf.size() > 0*/){
                initialization_kitti();
                initialized = true;
                p_ins_eskf->specify_init_state(init_state,uwb_msg_2_data(uwb_buf.back()));
                

                //TODO DEBUG
                //gps_buf.back().latitude,gps_buf.back().longitude,gps_buf.back().altitude);
...
*/
//UWB信号对应的消息msgs
void Ins_eskf_ROS_Wrapper::uwb_cb(const nav_msgs::OdometryConstPtr& uwb_in){
    nav_msgs::Odometry uwb_in_ros(*uwb_in);

    mtx.lock();
    uwb_buf.push_back(uwb_in_ros);
    mtx.unlock();
...
void Ins_eskf_ROS_Wrapper::initialization_kitti(){
    ...
    */
    ...
    double uwb_data_stamp = uwb_buf.back().header.stamp.toSec();
    sensor_msgs::Imu imu_data_;
    int imu_data_index = 0;
    for(; imu_data_index < imu_buf.size();imu_data_index++){
        if(imu_buf[imu_data_index].header.stamp.toSec() >= uwb_data_stamp){
            break;
        }
    }
    ...
    CHECK(imu_buf[imu_data_index - 1].header.stamp.toSec() < uwb_data_stamp) << "imu_buf[imu_data_index - 1].header.stamp.toSec() >= uwb_data_stamp,initialization failed.";
    float imu_data_inverval_ = imu_buf[imu_data_index].header.stamp.toSec() - imu_buf[imu_data_index-1].header.stamp.toSec();
    float front_propotion = (uwb_data_stamp - imu_buf[imu_data_index - 1].header.stamp.toSec())/imu_data_inverval_;
    ...
    initialization_stamp = uwb_data_stamp;
...
bool Ins_eskf_ROS_Wrapper::synce_measure(){
    ...
    */
    ...
    if(uwb_buf.empty() || imu_buf.empty()){
        LOG(INFO) << "imu uwb 数据集 空.";
        return false;
    }
    /*
    ...
    double recent_uwb_stamp = uwb_stamp_synce(uwb_buf.front().local_time);//uwb的时间戳还有待商榷
    if(imu_stamp_synce(imu_buf.back().header.stamp.toSec()) < recent_uwb_stamp){
        LOG(INFO) << "imu 时间戳还没赶上 uwb.";
        return false;
    }
    */
    double recent_uwb_stamp = uwb_buf.front().header.stamp.toSec();
    if(imu_buf.back().header.stamp.toSec() < recent_uwb_stamp){
        return false;
    }
    ...
    */
    ...
    while(imu_buf.front().header.stamp.toSec() < recent_uwb_stamp){
        measure.imu_buf.push_back(imu_msg_2_data(imu_buf.front()));
        last_imu_msg = imu_buf.front();
        imu_buf.pop_front();
    }
    /*
    double imu_stamp_text = imu_stamp_synce(imu_buf.front().header.stamp.toSec());
    //获取上次侧脸时刻到当前测量时刻之间的imu数据
    while(imu_stamp_synce(imu_buf.front().header.stamp.toSec()) <= recent_uwb_stamp){
        measure.imu_buf.push_back(imu_msg_2_data(imu_buf.front()));
        //last_imu_msg = imu_buf.front();
        imu_buf.pop_front();
    }
    */
    ...
    uwb_odom_msg_.header.stamp = uwb_buf.back().header.stamp;
    uwb_odom_msg_.header.frame_id = "odom";
    uwb_odom_msg_.child_frame_id = "uwb";
    uwb_odom_msg_.pose.pose.position.x = uwb_buf.back().pose.pose.position.x;
    uwb_odom_msg_.pose.pose.position.y = uwb_buf.back().pose.pose.position.y;
    uwb_odom_msg_.pose.pose.position.z = uwb_buf.back().pose.pose.position.z;

    //measure.gps_data = gps_msg_2_data(gps_buf.front());
    measure.uwb_data = uwb_msg_2_data(uwb_buf.front());
    //pub_uwb_odometry.publish(float_to_stdmsgs_Float64(measure.uwb_data.range));
    //gps_buf.pop_front();
    uwb_buf.pop_front();
...
Ins_eskf::UWB_data Ins_eskf_ROS_Wrapper::uwb_msg_2_data(nav_msgs::Odometry _uwb_msg){

    Ins_eskf::UWB_data uwb_data_;

    uwb_data_.stamp = _uwb_msg.header.stamp.toSec();
    uwb_data_.position << _uwb_msg.pose.pose.position.x,
                        _uwb_msg.pose.pose.position.y,
                        _uwb_msg.pose.pose.position.z;
...
void Ins_eskf_ROS_Wrapper::visualize_res_and_kitti_gps_magnetormeter(){
    ...
    */
    ...
    pub_uwb_odometry.publish(uwb_odom_msg_);
